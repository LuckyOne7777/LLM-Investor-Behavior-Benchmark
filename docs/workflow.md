...
LIBB exposes all internal state directly.  
There is no hidden abstraction layer between the user and the model state.

To access class variables, simply pass the `LIBBmodel` object into your function
and reference attributes directly.

This design allows prompt logic to remain fully flexible and transparent.

---

**IMPORTANT NOTE**

LIBB does **not** enforce strict protections on internal variables.

There are no error checks preventing you from overwriting critical attributes.
You should only modify class variables if you explicitly understand the
downstream consequences.

Accidentally mutating core state may silently break the workflow.
...
All variables used internally by LIBB are accessible in this way and may be
used to construct prompts, reports, or downstream logic.
